...
// static bool need_full_modem_reset = true;
...
static void modem_net_quiesce_ppp_link(void);
...
static void modem_net_conn_mgr_log_debug(void *ctx, const char *fmt, ...) 
{
	(void)ctx;
    va_list args;
    va_start(args, fmt);
...
static int modem_net_conn_mgr_connect_locked(struct modem_net_conn_mgr_ctx *ctx, int timeoutSeconds)
{
	...
	ops.print = modem_net_conn_mgr_log_debug;
	ops.error = modem_net_conn_mgr_log_debug;
...
static int modem_net_configure_context(void *ctx, const struct modem_net_profile *prof)
{
	...
	/* Skip CFUN=4/1 radio reset if modem is already registered.
	 * On reconnect the modem keeps its registration after PPP hangup (ATH).
	 * Forcing CFUN=4/1 deregisters and re-registers the modem, which can
	 * take longer than the CEREG polling window allows, causing -ETIMEDOUT.
	 */
	{
		char cereg_pre[MODEM_NET_AT_RESPONSE_SIZE];
		int pre_ret = modem_net_send_at("AT+CEREG?", cereg_pre, sizeof(cereg_pre));
		bool already_registered = (pre_ret == 0) &&
					   ((strstr(cereg_pre, "0,1") != NULL) ||
					    (strstr(cereg_pre, "0,5") != NULL));
		...
		if (!already_registered) {
			...
			modem_net_maybe_shell_print(sh, "Modem already registered, skipping CFUN=4/1 reset");
		}
	}
	...
	int registration_attempts = 30; // max 30secs
...
static int modem_net_escape_and_hangup(void)
{
	...
	/* Stop PPP traffic before sending the escape sequence. If PPP remains
	 * attached, residual frames can violate the guard time and the modem will
	 * stay in data mode, causing all later AT commands to time out.
	 */
	modem_net_quiesce_ppp_link();
